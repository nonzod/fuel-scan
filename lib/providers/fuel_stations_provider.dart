  ...
  void _applyFilters() {
    _filteredStations = _stations.where((station) {
      ...
      final price = station.prices.firstWhere(
        (p) => p.fuelType.toLowerCase().contains(fuelTypeStr) &&
    ...
    switch (_sortOption) {
      ...
      case SortOption.price:
        final fuelTypeStr = _selectedFuelType.toString().split('.').last;
        _filteredStations.sort((a, b) {
          final priceA = a.getPriceByFuelType(fuelTypeStr)?.price ?? double.infinity;
          final priceB = b.getPriceByFuelType(fuelTypeStr)?.price ?? double.infinity;
